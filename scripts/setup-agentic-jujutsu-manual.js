...
/**
 * Setup Agentic Jujutsu for Orchestra OS (Manual Install)
 *
 * Run this AFTER manually installing: pnpm add -D agentic-jujutsu
 */
...
// Check if agentic-jujutsu is installed
try {
  await import('agentic-jujutsu');
  console.log('✅ agentic-jujutsu is installed\n');
} catch (err) {
  console.error('❌ agentic-jujutsu not found!');
  console.error('   Please install it first:');
  console.error('   pnpm add -D agentic-jujutsu\n');
  console.error('   Or with npm:');
  console.error('   npm install --save-dev agentic-jujutsu --legacy-peer-deps\n');
  process.exit(1);
}
...
fs.writeFileSync(path.join(__dirname, 'jj-helper.js'), devHelperScript);
